...
agent = Agent(
    name="tumorboard_radiation",
    seed="tumorboard_radiation_seed_2026",
    port=8004,
    mailbox=True,
...
async def handle_message(ctx: Context, sender: str, msg: ChatMessage):
    ...
    try:
        r = client.chat.completions.create(
            model="asi1",
            messages=[
                {"role": "system", "content": """You are an expert radiation oncologist on a tumor board.
Propose a radiation treatment plan and return ONLY valid JSON with fields:
recommended_modality, target_volume_concept, total_dose_concept, organs_at_risk,
treatment_timing, concurrent_chemo, concurrent_agent, caution_zones, radiation_note."""},
                {"role": "user", "content": f"Propose radiation plan for: {json.dumps(tumor_ctx)}"},
            ],
            max_tokens=1000,
        )
