...
agent = Agent(
    name="tumorboard_oncology",
    seed="tumorboard_oncology_seed_2026",
    port=8002,
    mailbox=True,
...
async def handle_message(ctx: Context, sender: str, msg: ChatMessage):
    ...
    try:
        ...
        tumor_ctx = {"volume_cc": 28.4, "location": "right temporal lobe"}
    response = "Something went wrong"
    try:
        r = client.chat.completions.create(
            model="asi1",
            messages=[
                {"role": "system", "content": """You are an expert neuro-oncologist on a tumor board.
Given radiology and patient data, return ONLY valid JSON with fields:
likely_diagnosis, differential_diagnoses, recommended_biomarkers, treatment_options,
prognosis_context, urgency."""},
                {"role": "user", "content": f"Provide oncology assessment for: {json.dumps(tumor_ctx)}"},
            ],
            max_tokens=1000,
        )
